...
app.get('/CoordinatorDash', async (req, res) => {
    try {
...
app.get('/downloadQuestionPaper', async (req, res) => {
    try {
        const hodAssigns = await HodAssign.find();
        
        // Create a new Excel workbook and worksheet
        const workbook = new ExcelJS.Workbook();
        const worksheet = workbook.addWorksheet('Question Papers');
        ...
        hodAssigns.forEach(hodAssign => {
            worksheet.addRow({
                teacherName: hodAssign.teacherName,
                teacherEmail: hodAssign.teacherEmail,
                teacherDepartment: hodAssign.teacherDepartment,
                semester: hodAssign.semester,
                year: hodAssign.year,
                submissionStatus: hodAssign.submissionStatus
            });
        });
        
        // Set response headers for Excel file download
        res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
        res.setHeader('Content-Disposition', 'attachment; filename=question_papers.xlsx');
        
        // Write the workbook to the response
        await workbook.xlsx.write(res);
        res.end();
    } catch (error) {
        console.error('Error downloading question papers:', error);
        res.status(500).send('Internal Server Error');
    }
});
